...
class PingOneProvisionService {
    ...
    /**
     * Make authenticated request to PingOne Management API using a full absolute URL.
     * Use for endpoints that require the /v1/environments/{envId} path (schemas, attributes).
     */
     ...
     */
    async getSchemaId() {
        const url = `https://api.pingone.${this.region}/v1/environments/${this.envId}/schemas`;
        const response = await this.makeAbsoluteRequest('GET', url);
    ...
    /**
     * Create a custom attribute on the User schema (idempotent — skips if exists)
     */
    async createCustomAttribute(schemaId, name, description = '') {
        const listUrl = `https://api.pingone.${this.region}/v1/environments/${this.envId}/schemas/${schemaId}/attributes`;
        try {
            const existing = await this.makeAbsoluteRequest('GET', listUrl);
            ...
            /* probe failed — attempt create anyway */ }

        const response = await this.makeAbsoluteRequest('POST', listUrl, {
            name,
            type: 'STRING',
            enabled: true,
            unique: false,
    ...
    /**
     * Add a SPEL token claim mapping to an OIDC application (idempotent — skips if exists)
     */
    async addAttributeMapping(appId, claimName, spelExpression) {
        const listUrl = `https://api.pingone.${this.region}/v1/environments/${this.envId}/applications/${appId}/attributes`;
        try {
            const existing = await this.makeAbsoluteRequest('GET', listUrl);
            const attrs = existing.data._embedded?.attributes || [];
        ...
        await this.makeAbsoluteRequest('POST', listUrl, {
            name: claimName,
    ...
    async getApplicationSecret(appId) {
        const url = `https://api.pingone.${this.region}/v1/environments/${this.envId}/applications/${appId}/secret`;
        try {
            const response = await this.makeAbsoluteRequest('GET', url);
    ...
    /**
     * Run a headless client_credentials smoke test for a single worker app
     */
    ...
    /**
     * Write .env file (merge — only overwrites keys we generate, preserves existing)
     * Also persists to configStore SQLite for live-reload without server restart.
     */
    async writeEnvFile(config, provisioned) {
        const envPath = path.join(process.cwd(), '.env');

        if (!config.isVercel) {
            // Read existing .env (if any) and merge
            let existing = {};
            try {
            ...
            // Parse generated content and merge into existing
        ...
        // Persist to configStore SQLite for live reload (always, regardless of platform)
        let configStore;
        try {
        ...
        if (configStore) {
            const pairs = {
                ...
                pingone_mfa_acr_value: config.stepUpAcrValue
            };
    ...
    async provisionEnvironment(config, onStep) {
        ...
        try {
            ...
            steps.push({
                step: 'validate',
                icon: '✅',
                message: 'Worker credentials validated'
            });
            onStep(steps[steps.length - 1]);
            ...
            steps.push({
                step: 'population',
                icon: '👥',
                message: `Using population: ${this.populationId}`
            });
            onStep(steps[steps.length - 1]);
            ...
            steps.push({
                ...
                icon: '🏗️',
                message: 'Creating resource server...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (resourceResult.exists) {
                ...
                steps.push({
                    step: 'resource-server',
                    icon: '✅',
                    message: 'Resource server created'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            steps.push({
                ...
                icon: '🔧',
                message: 'Creating MCP resource server for admin operations...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (mcpResourceResult.exists) {
                ...
                steps.push({
                    step: 'mcp-resource-server',
                    icon: '✅',
                    message: 'MCP resource server created'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            steps.push({
                step: 'mcp-scopes',
                icon: '🎯',
                message: 'Creating MCP-specific scopes...'
            });
            onStep(steps[steps.length - 1]);

            const mcpScopes = [{
                    name: 'admin:read',
                    description: 'Read administrative data and system status'
                },
                {
                    name: 'admin:write',
                    description: 'Modify administrative settings and configurations'
                },
                {
                    name: 'admin:delete',
                    description: 'Delete users and administrative resources'
                },
                {
                    name: 'users:read',
                ...
                {
                    name: 'banking:read',
                    description: 'Read banking data and transaction history'
                },
                {
                    name: 'banking:write',
                    description: 'Perform banking operations and transfers'
                },
                {
                    name: 'banking:ai:agent:read',
                    description: 'Agent invocation permission'
                }
            ];
            ...
            if (failedMcpScopes > 0) {
                ...
                steps.push({
                    step: 'mcp-scopes',
                    icon: '✅',
                    message: `Created ${createdMcpScopes} MCP scopes`
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            steps.push({
                step: 'scopes',
                icon: '🎯',
                message: 'Creating banking scopes...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (failedScopes > 0) {
                ...
                steps.push({
                    step: 'scopes',
                    icon: '✅',
                    message: `Created ${createdScopes} scopes`
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            steps.push({
                step: 'admin-app',
                icon: '🔧',
                message: 'Creating admin application...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (adminAppResult.exists) {
                ...
                steps.push({
                    step: 'admin-app',
                    icon: '✅',
                    message: 'Admin application created'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            if (!adminAppResult.exists) {
                steps.push({
                    step: 'admin-config',
                    icon: '⚙️',
                    message: 'Configuring admin application...'
                });
                onStep(steps[steps.length - 1]);
                ...
                steps.push({
                    step: 'admin-config',
                    icon: '✅',
                    message: 'Admin application configured with token exchange'
                });
                onStep(steps[steps.length - 1]);
            }
            ...
            steps.push({
                step: 'admin-grants',
                icon: '🔑',
                message: 'Granting scopes to admin application...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (adminGrantResult.success && adminMcpGrantResult.success) {
                steps.push({
                    step: 'admin-grants',
                    icon: '✅',
                    message: 'Admin scopes granted from both resource servers'
                });
            } else {
                steps.push({
                    step: 'admin-grants',
                    icon: '⚠️',
                    message: 'Failed to grant some admin scopes'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            steps.push({
                step: 'user-app',
                icon: '👤',
                message: 'Creating user application...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (userAppResult.exists) {
                ...
                steps.push({
                    step: 'user-app',
                    icon: '✅',
                    message: 'User application created'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            if (!userAppResult.exists) {
                steps.push({
                    step: 'user-config',
                    icon: '⚙️',
                    message: 'Configuring user application...'
                });
                onStep(steps[steps.length - 1]);
                ...
                steps.push({
                    step: 'user-config',
                    icon: '✅',
                    message: 'User application configured'
                });
                onStep(steps[steps.length - 1]);
            }
            ...
            steps.push({
                step: 'user-grants',
                icon: '🔑',
                message: 'Granting scopes to user application...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (userGrantResult.success) {
                steps.push({
                    step: 'user-grants',
                    icon: '✅',
                    message: 'User scopes granted'
                });
            } else {
                steps.push({
                    step: 'user-grants',
                    icon: '⚠️',
                    message: 'Failed to grant user scopes'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            steps.push({
                step: 'bankuser',
                icon: '👨',
                message: 'Creating demo user: bankuser...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (!bankUserResult.exists) {
                ...
                steps.push({
                    step: 'bankuser-password',
                    icon: '🔒',
                    message: 'Setting bankuser password...'
                });
                onStep(steps[steps.length - 1]);

                await this.setUserPassword(bankUserResult.user.id, bankUserPassword);
                provisioned.bankUser = {
                    ...bankUserResult.user,
            ...
            steps.push({
                step: 'bankadmin',
                icon: '👨‍💼',
                message: 'Creating demo user: bankadmin...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (bankAdminResult.exists) {
                ...
                steps.push({
                    step: 'bankadmin',
                    icon: '✅',
                    message: 'User bankadmin created'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            if (!bankAdminResult.exists) {
                ...
                steps.push({
                    step: 'bankadmin-password',
                    icon: '🔒',
                    message: 'Setting bankadmin password...'
                });
                onStep(steps[steps.length - 1]);

                await this.setUserPassword(bankAdminResult.user.id, bankAdminPassword);
                provisioned.bankAdmin = {
                    ...bankAdminResult.user,
                ...
                steps.push({
                    step: 'bankadmin-password',
                    icon: '✅',
                    message: 'Bankadmin password set'
                });
                onStep(steps[steps.length - 1]);
            } else {
                provisioned.bankAdmin = {
            ...
            steps.push({
                step: 'mcp-app',
                icon: '🤖',
                message: 'Creating MCP Server application...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (mcpAppResult.exists) {
                ...
                steps.push({
                    step: 'mcp-app',
                    icon: '✅',
                    message: 'MCP Server application created'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            if (!mcpAppResult.exists) {
                steps.push({
                    step: 'mcp-config',
                    icon: '⚙️',
                    message: 'Configuring MCP Server application...'
                });
                onStep(steps[steps.length - 1]);
                ...
                steps.push({
                    step: 'mcp-config',
                    icon: '✅',
                    message: 'MCP Server application configured'
                });
                onStep(steps[steps.length - 1]);
            }
            ...
            steps.push({
                step: 'mcp-grants',
                icon: '🔑',
                message: 'Granting scopes to MCP Server application...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (mcpAppGrantResult.success) {
                steps.push({
                    step: 'mcp-grants',
                    icon: '✅',
                    message: 'MCP Server scopes granted'
                });
            } else {
                steps.push({
                    step: 'mcp-grants',
                    icon: '⚠️',
                    message: 'Failed to grant MCP Server scopes'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            steps.push({
                step: 'worker-app',
                icon: '🔧',
                message: 'Creating Worker application...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (workerAppResult.exists) {
                ...
                steps.push({
                    step: 'worker-app',
                    icon: '✅',
                    message: 'Worker application created'
                });
            }
            onStep(steps[steps.length - 1]);
            ...
            if (!workerAppResult.exists) {
                steps.push({
                    step: 'worker-config',
                    icon: '⚙️',
                    message: 'Configuring Worker application...'
                });
                onStep(steps[steps.length - 1]);
                ...
                steps.push({
                    step: 'worker-config',
                    icon: '✅',
                    message: 'Worker application configured'
                });
                onStep(steps[steps.length - 1]);
            }
            ...
            steps.push({
                step: 'worker-grants',
                icon: '🔑',
                message: 'Granting scopes to Worker application...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (workerAppGrantResult.success) {
                steps.push({
                    step: 'worker-grants',
                    icon: '✅',
                    message: 'Worker scopes granted'
                });
            } else {
                steps.push({
                    step: 'worker-grants',
                    icon: '⚠️',
                    message: 'Failed to grant Worker scopes'
                });
            }
            onStep(steps[steps.length - 1]);

            // Step 20.5: SPEL Attribute Mapping
            onStep({
                step: 'spel-start',
                icon: '🔗',
                message: 'Setting up bankingPrincipalUserId attribute mapping...'
            });
            try {
                ...
                onStep({
                    step: 'spel-attribute',
                    icon: '✅',
                    message: attrResult.existed ?
                        'bankingPrincipalUserId attribute already exists' :
                        'Created bankingPrincipalUserId custom attribute'
                });

                const spelExpression = '${user.bankingPrincipalUserId}';
                const appsToMap = [{
                        id: provisioned.adminApp?.id,
                        name: 'Admin App'
                    },
                    {
                        id: provisioned.userApp?.id,
                        name: 'User App'
                    }
                ].filter(a => a.id);

                for (const app of appsToMap) {
                    const mapResult = await this.addAttributeMapping(app.id, 'bankingPrincipalUserId', spelExpression);
                    onStep({
                        step: 'spel-mapping',
                        icon: '✅',
                        message: `bankingPrincipalUserId ${mapResult.existed ? 'already mapped' : 'mapped'} on ${app.name}`
                    });
                }
            } catch (spelErr) {
                onStep({
                    step: 'spel-warning',
                    icon: '⚠️',
                    message: `SPEL setup skipped (non-fatal): ${spelErr.message}`
                });
            }
            ...
            steps.push({
                step: 'config',
                icon: '📝',
                message: config.isVercel ? 'Setting Vercel environment variables...' : 'Writing .env file...'
            });
            onStep(steps[steps.length - 1]);
            ...
            if (config.isVercel) {
                ...
                steps.push({
                    step: 'config',
                    icon: '✅',
                    message: `.env file written to ${envPath}`
                });
            }
            onStep(steps[steps.length - 1]);
        ...
        try {
            switch (type) {
                case 'resource':
                    ...
                    return {
                        success: true, message: 'Resource server deleted'
                    };
