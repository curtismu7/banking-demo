...
export default function AuditPage() {
  ...
  // Floating window position + size — centred on first render
  ...
  const resizeRef = useRef(null); // { dir, startX, startY, origX, origY, origW, origH }

  // --- Mouse move / up listeners (attached to window so they survive cursor leaving element) ---
  useEffect(() => {
    const onMove = (e) => {
      if (dragRef.current) {
        const { startX, startY, origX, origY } = dragRef.current;
        // No clamping — allow dragging off-screen
  ...
  return (
    ...
    >
      ...
      <div className="audit-float-content">
        {/* Summary stats */}
        {summary && (
