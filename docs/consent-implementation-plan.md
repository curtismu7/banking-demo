...
BFF code
[ ] oauthUser.js — store consentAcr + mayAct in session on callback
[ ] routes/auth.js — add consentGiven + consentAcr + mayAct to user status response
[ ] routes/auth.js — add GET /api/auth/consent-url endpoint
[ ] agentMcpTokenService.js — block exchange with AGENT_CONSENT_REQUIRED when
      consentGiven=false AND may_act absent
[ ] MCP route — catch AGENT_CONSENT_REQUIRED, return 403 with consentUrl
[ ] Set AGENT_CONSENT_ACR env var

UI code
[ ] BankingAgent.js — fetch consentGiven from user status on panel open
[ ] BankingAgent.js — add ba-consent-required banner (amber, distinct from red consent challenge)
[ ] BankingAgent.js — disable action buttons when !consentGiven
[ ] BankingAgent.js — add handleGrantConsent → redirect to /api/auth/consent-url
[ ] BankingAgent.css — add .ba-consent-required styles
[ ] agentMcpTokenService.js — add acr + consentGiven to User Token event
[ ] TokenChainDisplay.js — add acr pill (green when given, amber when missing)
[ ] TokenChainDisplay.css — add .tcd-pill--consent styles
[ ] TokenChainDisplay.js — update legend
